...
public class SpellChecker2 {
    ...
    static final String hfre1= "aeir";
    static final String hfre2= "cs";
    static final String hfre3="krth";
    ...
    public double calculateLM(String word) {
        ...
        //
    	double total=0;
    	Object[] nums=dict.values().toArray();
    	//calculate the total # of the words
    	for(int i=0; i< nums.length; i++){
    		total+=Integer.valueOf(nums[i].toString());
    	}
    	double fre=dict.get(word)/total;
        return dict.containsKey(word) ? fre : 0.0;
    }

    /*
        calculateLM: returns the error model P(word|correction)
        Input: word, correction, and the editDistance between the two 
        Output: P(word|correction)
     */
    public double calculateEM(String word, String correction, double editDistance) {
        // For Task 1, we simply return 1 as the error model P(word|correction)
    	int l1=word.length();
		int l2=correction.length();
    	if(editDistance==0) return 1.0;
    	if(editDistance==1){
    		if(l1==l2){
    			int i=0;
    			while(word.charAt(i)==correction.charAt(i)){  //find the index of the different char
    				i++;
    			}
    			if(i==l1-1||word.charAt(i+1)==correction.charAt(i+1)){    //substitution
    				if(hfre1.indexOf(word.charAt(i))!=-1&&hfre1.indexOf(correction.charAt(i))!=-1)
    					return 0.8*0.31*0.9;
    				if(hfre2.indexOf(word.charAt(i))!=-1&&hfre2.indexOf(correction.charAt(i))!=-1)
    					return 0.8*0.31*0.9;
    				return 0.8*0.31*0.1;
    			}	
    			return 0.8*0.1; //transposition
    		}
    		if(l1<l2) {    //Delection
    			if(word.charAt(l1-1)=='y'&&word.charAt(l1-2)=='l') return 0.8;
    			if(hfre3.indexOf(word.charAt(l1-1))!=-1&&correction.charAt(l2-1)=='e') return 0.8;
    			return 0.8*0.6;
    		}
    		return 0.8*0.1;	//insertion
    	}
    	if(editDistance==2){
    ...
    */
    private Map < String, Double > EditDist1(String word) {
        ...
        for (int i = 0; i <= word.length(); ++i) {
            ...
            //"transposition between [i] and [i+1]": transpose [i-1] into [i], and [i] into [i-1]
            if(R.length() > 1){
            ...
            //"substitution at [i]": substitute [i] with another character.
            if(R.length() >0){
            	for(int j=0; j<alphabet.length(); j++){
